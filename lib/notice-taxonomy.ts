...
function getSchoolRangeText(
  project: Pick<PublicNoticeProject, 'schoolName' | 'tags'>
) {
  return [project.schoolName, ...(project.tags || [])].join(' ');
}

export function getSchoolRangeMatches(
  project: Pick<PublicNoticeProject, 'schoolName' | 'tags'>
) {
  ...
  if (
    /985|985工程/.test(text) ||
    KNOWN_985_SCHOOLS.some((name) => schoolName.includes(name))
  ) {
    ranges.add('985');
  }
...
export function matchesSchoolRange(
  project: Pick<PublicNoticeProject, 'schoolName' | 'tags'>,
  range: SchoolRangeFilter
) {
  ...
  if (range === '双一流') {
    return !ranges.has('985') && !ranges.has('211') && ranges.has('双一流');
  }
  return false;
}

export function inferSchoolRange(
  project: Pick<PublicNoticeProject, 'schoolName' | 'tags'>
) {
  const ranges = getSchoolRangeMatches(project);
