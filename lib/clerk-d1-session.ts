...
const guardedFetch:typeof fetch=async(input,init)=>{if(accountQuota.blocked())return Response.json({error:'SERVICE_QUOTA_EXCEEDED'},{status:402});const r=await fetch(input,init);if(r.status===402){const error=await r.clone().json().catch(()=>null) as {error?:string}|null;if(error?.error!=='FILL_LIMIT_REACHED')accountQuota.restrict();}return r;};
export function prepareExplicitD1SignInRetry(){
 ...
 // Discard only the request circuit, never the Clerk session or local records.
 client=null;
}
function getClient(){
