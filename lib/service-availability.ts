...
export function createAvailabilityFetch(fetcher: typeof fetch = (...args) => fetch(...args)) {
  const restrictedOrigins = new Map<string,ReturnType<typeof createQuotaCircuit>>();
  return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (restrictedOrigins.get(scope)?.blocked()) throw new ServiceUnavailableError(402);
    const response = await fetcher(input, init);
    if (response.status === 402) {const circuit=restrictedOrigins.get(scope)||createQuotaCircuit();circuit.restrict();restrictedOrigins.set(scope,circuit);}
    return response;
  };
}
