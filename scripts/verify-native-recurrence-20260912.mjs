...
const root=process.cwd(),exe=path.join(root,'src-tauri/target/release/seekoffer-desktop.exe'),out=path.join(root,'artifacts/desktop-recurrence-20260912');fs.mkdirSync(out,{recursive:true});
...
try{
 ...
 stage='native public transport';const native=await page.evaluate(async()=>{
  ...
  const started=performance.now();const list=await invoke('native_public_request',{path:'/api/public/notices/?page=1&pageSize=16&year=2026&sort=publish',method:'GET',body:null});
  const listMs=Math.round(performance.now()-started);const metaStart=performance.now();
  const parts=await Promise.all(['summary','facets','colleges'].map(section=>invoke('native_public_request',{path:'/api/public/notices/metadata/?'+new URLSearchParams({section,version:list.metadataVersion}),method:'GET',body:null})));
  if(!parts.every(p=>p.version===list.metadataVersion)||!parts[0].stats||!parts[1].facets||!Array.isArray(parts[2].collegeStats))throw Error('NATIVE_METADATA_INVALID');
  const metadataMs=Math.round(performance.now()-metaStart);const id=list.items?.[0]?.id;if(!id)throw Error('PUBLIC_LIST_EMPTY');
  ...
  return{listMs,metadataMs,metadataVersionsMatched:true,count:list.items.length,bytes:new TextEncoder().encode(JSON.stringify(list)).length,detailIdMatches:detail?.id===id,scopeRejected};
 });assert.equal(native.count,16);assert.ok(native.detailIdMatches&&native.scopeRejected);report.nativePublic=native;
 stage='native list UI';
 await page.goto('http://tauri.localhost/notices/',{waitUntil:'domcontentloaded'});
 await page.getByText(/^共 [\d,]+ 条结果$/).waitFor({timeout:25000});
 report.checks.push('native compiled list renders real list and all three metadata sections');
 await page.screenshot({path:path.join(out,'native-public-notices.png')});
