...
3. In Email:
   ...
   - For registration, edit the **Confirm sign up** template so the email shows
     the 6-digit code with `{{ .Token }}`. The frontend verifies it with
     `verifyOtp({ type: 'signup' })`.
   - For the login panel's 6-digit code tab, edit the **Magic Link** template so
     the email shows `{{ .Token }}`. The frontend verifies it with
     `verifyOtp({ type: 'email' })`.
   - Supabase sends a link when a template only uses `{{ .ConfirmationURL }}`,
     and sends a 6-digit OTP when it uses `{{ .Token }}`.
