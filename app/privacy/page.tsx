...
const privacySections = [
  ...
  {
    title: '匿名访问统计',
    body: '网站会进行匿名访问统计，并创建随机设备标识，记录页面路径、页面标题、来源页面、语言、时区、访问次数和在线状态，用于判断页面是否易用和发现运行异常。使用桌面端下载按钮时，下载统计仅记录按钮发起时间、桌面端版本和设备平台，不保存 IP 地址、账号信息或任何申请内容；该记录只表示用户发起下载，不代表安装包下载完成或软件安装完成。匿名统计不需要用户另行同意，不用于识别真实身份、广告画像、与本服务无关的商业营销或向无关第三方披露。'
  },
  {
    title: '本机保存与云端同步',
  ...
  return (
    <SiteShell>
      <PageSectionTitle
        ...
        title="隐私政策"
        subtitle="所有数据规则集中在这里说明：收集什么、为什么使用、保存多久，以及你如何管理自己的信息。更新日期：2026 年 9 月 3 日。"
      />
